{{-- Pourquoi nous choisir --}}

<section>
    <header class="d-flex justify-content-center text-center my-4">
        <h2 class="fs-1">Pourquoi nous choisir</h2>
    </header>
    <main class="pt-3">
        <div class="row">
            <div class="col-12 col-md-4">
                <div class="card bg-transparent border-0 shadow-none">
                    {{-- <i class="fa fa-shield card-img-top text-center" aria-hidden="true" style="font-size: 82px"></i> --}}
                    <div class="d-flex justify-content-center align-items-center">
                        <img src="{{ asset('images/shield-rounded.png') }}" alt="" class="card-img-top text-center" style="height: 82px; width: 82px;">
                    </div>
                    <div class="card-body">
                        <div class="card-text">
                            <h3 class="text-center">Sécurité garantie</h3>
                            <p class="text-center">
                                Nous mettons tout en
                                œuvre pour vous proposer
                                une plateforme sécurisée.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-12 col-md-4">
                <div class="card bg-transparent border-0 shadow-none">
                    {{-- <i class="fa fa-cube card-img-top text-center" aria-hidden="true" style="font-size: 82px"></i> --}}
                    <div class="d-flex justify-content-center align-items-center">
                        <img src="{{ asset('images/easy-installation-rounded.png') }}" alt="" class="card-img-top text-center" style="height: 82px; width: 82px;">
                    </div>
                    <div class="card-body">
                        <div class="card-text">
                            <h3 class="text-center">Simplicité</h3>
                            <p class="text-center">
                                Facile d'utilisation et
                                dotée de technologies
                                avancées
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-12 col-md-4">
                <div class="card bg-transparent border-0 shadow-none">
                    {{-- <i class="fa fa-commenting-o card-img-top text-center" aria-hidden="true" style="font-size: 82px"></i> --}}
                    <div class="d-flex justify-content-center align-items-center">
                        <img src="{{ asset('images/comments-rounded.png') }}" alt="" class="card-img-top text-center" style="height: 82px; width: 82px;">
                    </div>
                    <div class="card-body">
                        <div class="card-text">
                            <h3 class="text-center">Système d'avis libre</h3>
                            <p class="text-center">
                                Un processus sécurisé de
                                collecte, de modération et
                                de diffusion des avis
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </main>
</section>
